...
**实习日期：2026年9月4日**
**实习内容： Harmony OS 物联网应用开发实战-3——OLED 视频播放、智能夜灯与智能环境监测**

今天继续进行 Harmony OS 物联网应用开发实战，主要完成了 OLED 屏幕显示与视频播放、智能夜灯以及智能环境监测等综合案例。与前两天学习单个外设接口相比，今天的案例涉及传感器、显示屏、网络通信和移动端应用之间的协同，系统综合性明显增强，也让我对物联网应用从数据采集到交互控制的完整流程有了更深入的认识。

首先，我继续学习了 `12_ssd1306` OLED 显示示例。该示例需要使用鸿蒙 IoT 硬件子系统的 I2C 接口，因此在编译前需要检查 Hi3861 SDK 的配置文件，确认 `CONFIG_I2C_SUPPORT=y` 已经启用。随后在 `demo/BUILD.gn` 中启用 `12_ssd1306:app` 编译目标，重新选择 `wifiiot_hispark_pegasus` 开发板并完成编译、烧录和测试。程序运行后，OLED 能够依次显示英文、中文、位图、不同字号以及方框、箭头和圆形等图形，使我进一步理解了 SSD1306 屏幕缓冲区和图形绘制接口的使用方式。

在此基础上，我完成了 `13_oledplayer` OLED 视频播放案例。由于 OLED 无法直接播放普通 MP4 文件，需要先对素材进行预处理。我先使用 FFmpeg 将视频转换为约 10 帧每秒的低帧率素材，再建立 Python 虚拟环境并安装 OpenCV，利用项目脚本把视频帧转换成开发板能够处理的 `out.bin` 数据。这个过程让我认识到，嵌入式设备受限于处理能力、存储空间和屏幕规格，桌面端常见的媒体资源必须经过降帧、缩放、灰度化和二进制编码等处理后才能使用。

视频数据通过 TCP 服务传输给 Hi3861。为了使服务启动和文件访问更加方便，我在物理机上准备 Python 运行环境并启动视频案例自带的 TCP 服务，然后修改开发板程序中的 Wi-Fi 热点信息和服务器地址。固件重新编译并烧录后，开发板先连接无线热点，再主动连接电脑上的 TCP 服务接收视频帧。通过观察热点连接设备、串口中的 Wi-Fi 和数据传输日志以及 OLED 实际画面，最终确认视频能够在屏幕上连续播放。这个案例把素材转换、网络传输和显示驱动串联起来，使我对应用层数据如何经过网络送到硬件显示设备有了直观认识。

随后，我学习并运行了 `29_smart_night_light` 智能夜灯案例。该案例同时包含南向硬件程序和北向移动端软件。我将所需示例代码整理到工程中，修改 `demo/BUILD.gn` 启用对应编译目标，完成固件编译和烧录。设备复位后，手机先连接开发板建立的 `HarmonyOS-AP` 热点，再通过配套应用向板端下发实际无线网络的名称和密码；板端重启并连接新热点后，手机应用即可与设备通信。在智能夜灯界面中，可以调节灯光亮度和保持时间，设备则结合环境光线和人体感应结果控制照明。通过这一流程，我理解了设备配网、参数下发以及移动端远程控制之间的关系。

最后，我对 `30_samart_environmental_monitoring_system` 智能环境监测案例进行了配置、编译、烧录和测试。该案例在前面温湿度、光照、显示等基础功能之上进一步加入网络和应用交互，是一个更完整的物联网终端示例。通过修改构建配置并运行该程序，我进一步体会到综合项目不仅需要关注单个驱动是否正常，还需要保证多个传感器、任务、通信模块和显示逻辑之间能够正确协作。

通过今天的实践，我走通了“资源预处理—固件编译—开发板联网—数据传输—本地显示”和“设备配网—传感器感知—移动端控制”两类典型物联网工作流。相比单独调用 GPIO、I2C 或 UART 接口，这些综合案例更能体现物联网系统的整体结构，也为下一步学习 MQTT 协议和云平台接入打下了基础。

## 实习日志——第八天

**实习日期：2026年9月5日**
**实习内容： MQTT 协议、EMQX 服务搭建与华为云 IoT 平台接入**

今天主要学习了 MQTT 消息通信协议，并完成了 EMQX MQTT 服务搭建以及华为云 IoT 平台的产品、设备和数据转发配置。在前一天实现开发板局域网连接和 TCP 数据传输的基础上，今天进一步接触了面向物联网设备的发布/订阅通信模式，并尝试将 Hi3861 的传感器数据和控制功能接入云端平台。

首先，我学习了 MQTT 的基本原理。MQTT 是一种构建在 TCP/IP 之上的轻量级消息传输协议，采用发布/订阅模式，由发布者、订阅者和消息代理 Broker 共同完成通信。发布者和订阅者不需要直接建立业务上的一对一联系，而是通过主题 Topic 交换消息，Broker 负责匹配主题并转发负载 Payload，因此可以降低设备与应用之间的耦合。MQTT 协议报文由固定头、可变头和消息体组成，并提供“至多一次”“至少一次”和“只有一次”三种 QoS 服务质量，适合带宽较低、网络不稳定且计算资源有限的物联网设备。

为了验证 MQTT 通信机制，我使用 Docker 部署了 EMQX 服务，并配置了 Broker 和管理控制台所需的端口映射。容器启动后，我检查了 EMQX 与 OpenHarmony 编译容器的运行状态，并通过浏览器访问 EMQX Dashboard。通过这个过程，我对 MQTT Broker 在系统中的作用有了更直接的认识：设备和应用都作为客户端连接 Broker，通过发布或订阅指定主题完成一对多消息分发，而不需要自行维护复杂的连接关系。

随后，我进入华为云物联网平台进行云端配置。首先创建 IoT 接入实例，并确认实例所在区域以及设备接入地址；然后通过域名解析获取接入服务器信息，为开发板后续建立 MQTT 连接做准备。在平台中，我创建了“智慧农业”产品并注册对应设备，同时保存设备连接所需的标识和认证信息。实际记录中仅保留配置位置和用途，不在日志中写入设备密钥，以避免认证信息泄露。

在产品模型配置部分，我为智慧农业服务添加了温度、湿度、亮度、紫光灯状态和电机状态等属性，并设置了紫光灯控制和电机控制命令。属性用于描述设备向云端上报的状态数据，命令则用于平台向设备下发控制指令。通过配置数据类型、取值范围和命令参数，我进一步理解了物联网平台的产品模型相当于设备能力的统一描述，云端只有先明确设备能够上报什么、能够执行什么，才能正确解析数据并进行控制。

之后，我配置了数据转发规则和消息主题，使平台能够将设备消息按规则推送到指定 Topic，并准备了 MQTT 接入所需的凭证和设备参数。结合开发板代码，我梳理了服务器地址、设备 ID、认证信息、Wi-Fi 名称和密码等配置项在不同源码文件中的作用，并相应修改 MQTT 连接与网络配置。完成构建配置和 Makefile 调整后，使用 `hb set` 选择 `wifiiot_hispark_pegasus`，再通过 `hb build -f` 编译固件，随后将生成的固件烧录到开发板中进行测试。

测试过程中，我重点观察开发板连接 Wi-Fi、建立 MQTT 会话、向华为云上报传感器属性以及接收平台命令等环节。通过串口日志和云端设备状态，可以判断网络连接、身份认证、主题订阅及消息发布是否成功。这一过程让我认识到，云端接入问题往往涉及多个层次：无线网络是否可用、服务器地址和端口是否正确、设备凭证是否匹配、Topic 与产品模型是否一致，任何一个环节配置错误都可能导致设备无法上线或数据无法解析。

通过今天的学习，我完成了从本地 MQTT Broker 到华为云 IoT 平台的完整实践，对“设备—网络—消息代理—云平台—应用”这一物联网链路形成了整体认识。MQTT 的轻量级发布/订阅机制适合 Hi3861 这类资源受限设备，而云平台则提供设备管理、数据建模、状态查看、规则转发和远程控制能力。至此，前面学习的传感器采集、OLED 显示、Wi-Fi 联网和嵌入式编译烧录等知识被进一步整合到了云端物联网应用中。
