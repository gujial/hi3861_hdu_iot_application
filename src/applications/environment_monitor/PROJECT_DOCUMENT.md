...
当前固件已实现可配置的温湿度本地报警，以及标定后可显式启用的燃气电阻本地报警。燃气异常使用连续三短声，温湿度异常使用单次长声。为防止未经标定产生误导，设备端和 Web 端的燃气报警默认关闭。Web 阈值不会下发到开发板，因此本地阈值与远程阈值需要分别维护。
...
| FR-08 | 阈值管理 | 家人可按住宅实际情况设置三项指标的启用状态、上下限和恢复缓冲；非法修改整体拒绝 | 高 |
...
| `PUT /api/thresholds` | 原子更新全部阈值 | HTTP 200 `{"ok":true}` |
...
| P7 自动化测试 | 覆盖配置生成、阈值状态机、持久化、IAM、IoTDA 和 ntfy | 15 个自动化测试 | 已完成 |
...
当前自动化结果为设备端 8/8、Web 端 9/9 通过，TypeScript 检查、Web 生产构建和启用云功能的 Hi3861 固件构建通过。
...
| IT-06 远程高温告警 | Web 与 ntfy 可用，本人及家人手机已订阅 | 将 Web 温度上限临时调低至最新温度以下 | Web 产生一次 high 告警，目标手机收到 ntfy 推送；设备蜂鸣器不受 Web 阈值改变影响 | 待端到端验证 |
...
工程质量方面，配置生成器可以在编译前发现多类常见错误和非法本地报警阈值，自动化测试覆盖 17 个关键场景，Web 类型检查、生产构建和启用云功能的 Hi3861 固件构建已经通过。测试使用占位凭据和临时数据库，不读取真实设备密码。
