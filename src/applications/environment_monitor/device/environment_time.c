...
static int SyncServer(const char *server) {
  ...
  if (received == sizeof(response) && (response[0] & 7) == 4 &&
      (response[0] >> 6) != 3 && ((response[0] >> 3) & 7) >= 3 &&
      response[1] > 0 && response[1] < 16 &&
      memcmp(response + 24, request + 40, 8) == 0) {
    uint32_t seconds = (uint32_t)response[40] << 24 |
...
int EnvironmentSyncTime(void) {
  if (hi_systick_get_real_time() >= 1704067200)
    return 0;
  ...
  for (unsigned int i = 0; i < sizeof(servers) / sizeof(servers[0]); ++i) {
    if (i > 0 && strcmp(servers[i], ENV_NTP_SERVER) == 0)
      continue;
    if (SyncServer(servers[i]) == 0)
      return 0;
  }
  return -1;
}
