...
static unsigned int lastQueuedTick;
static EnvironmentThresholds currentThresholds = {ENV_LOCAL_TEMP_LOW,
                                                  ENV_LOCAL_TEMP_HIGH,
...
void EnvironmentCloudGetThresholds(EnvironmentThresholds *thresholds) {
  if (!thresholds)
    return;
  if (thresholdsMutex)
    osMutexAcquire(thresholdsMutex, osWaitForever);
  *thresholds = currentThresholds;
  if (thresholdsMutex)
    osMutexRelease(thresholdsMutex);
}
...
static int ValidThresholds(const EnvironmentThresholds *t) {
  return (t->temperatureEnabled == 0 || t->temperatureEnabled == 1) &&
         ...
         t->temperatureLow < t->temperatureHigh && t->humidityLow >= 0 &&
         t->humidityHigh <= 100 && t->humidityLow < t->humidityHigh &&
         t->gasLow >= 0 && t->gasHigh <= 1000000 && t->gasLow < t->gasHigh;
}
...
static int CommandArrived(void *context, char *topicName, int topicLen,
  ...
  const cJSON *name =
      root ? cJSON_GetObjectItemCaseSensitive(root, "command_name") : NULL;
  const cJSON *service =
      root ? cJSON_GetObjectItemCaseSensitive(root, "service_id") : NULL;
  const cJSON *paras =
      root ? cJSON_GetObjectItemCaseSensitive(root, "paras") : NULL;
  if (!request || (size_t)(request - topic) >= topicSize ||
      !cJSON_IsString(name) ||
      strcmp(name->valuestring, "SetThresholds") != 0 ||
      !cJSON_IsString(service) ||
      strcmp(service->valuestring, ENV_SERVICE_ID) != 0 ||
      !cJSON_IsObject(paras)) {
    handled = 0;
  } else {
    request += sizeof(marker) - 1;
    size_t remaining = topicSize - (size_t)(request - topic);
    if (remaining == 0 || remaining >= sizeof(command.requestId))
      handled = 0;
    else {
      memcpy(command.requestId, request, remaining);
      command.requestId[remaining] = '\0';
      handled =
          Boolean(paras, "temperature_enabled",
                  &command.thresholds.temperatureEnabled) &&
          Number(paras, "temperature_low",
                 &command.thresholds.temperatureLow) &&
          Number(paras, "temperature_high",
                 &command.thresholds.temperatureHigh) &&
          Boolean(paras, "humidity_enabled",
  ...
  if (handled && osMessageQueuePut(commandQueue, &command, 0, 0) != osOK)
    handled = 0;
  if (!handled)
    printf("[environment] invalid threshold command\n");
  cJSON_Delete(root);
...
static int ApplyCommand(MQTTClient client, const ThresholdCommand *command) {
  char saved[128];
  int savedLength = snprintf(
      saved, sizeof(saved), "%d,%.3f,%.3f,%d,%.3f,%.3f,%d,%.3f,%.3f",
      command->thresholds.temperatureEnabled,
      command->thresholds.temperatureLow, command->thresholds.temperatureHigh,
      command->thresholds.humidityEnabled, command->thresholds.humidityLow,
      command->thresholds.humidityHigh, command->thresholds.gasEnabled,
      command->thresholds.gasLow, command->thresholds.gasHigh);
  if (savedLength < 0 || (size_t)savedLength >= sizeof(saved) ||
      UtilsSetValue("env.thresholds", saved) != 0)
    return -1;
  if (thresholdsMutex)
    osMutexAcquire(thresholdsMutex, osWaitForever);
  ...
  if (topicLength < 0 || (size_t)topicLength >= sizeof(topic) ||
      payloadLength < 0 || (size_t)payloadLength >= sizeof(payload))
    return -1;
...
static int JoinWifi(int netId) {
  if (ConnectTo(netId) != WIFI_SUCCESS)
    return -1;
  for (int i = 0; i < 30 && !wifiConnected; ++i)
    sleep(1);
  if (!wifiConnected)
    return -1;
  struct netif *iface = netifapi_netif_find("wlan0");
  if (!iface || netifapi_dhcp_start(iface) != ERR_OK) {
    Disconnect();
    wifiConnected = 0;
    return -1;
  }
...
static void CloudTask(void *argument) {
  ...
  if (strcpy_s(wifi.ssid, sizeof(wifi.ssid), ENV_WIFI_SSID) != EOK ||
      strcpy_s(wifi.preSharedKey, sizeof(wifi.preSharedKey),
               ENV_WIFI_PASSWORD) != EOK)
    return;
  wifi.securityType = WIFI_SEC_TYPE_PSK;
  if (RegisterWifiEvent(&wifiEvents) != WIFI_SUCCESS ||
      EnableWifi() != WIFI_SUCCESS ||
  ...
  for (;;) {
    if (!wifiConnected && JoinWifi(netId) != 0) {
      sleep(5);
      continue;
    }
    if (EnvironmentSyncTime() != 0) {
      printf(
          "[environment] time synchronization failed; retrying before TLS\n");
      sleep(5);
      continue;
    }
    ...
    const cert_string ca = {(const unsigned char *)ENV_MQTT_CA_PEM,
                            sizeof(ENV_MQTT_CA_PEM)};
    ...
    if (rc == MQTTCLIENT_SUCCESS) {
      rc = MQTTClient_subscribe(
          client, "$oc/devices/" ENV_DEVICE_ID "/sys/commands/#", 1);
    }
    if (rc == MQTTCLIENT_SUCCESS) {
      backoff = 2;
      ...
      while (wifiConnected && MQTTClient_isConnected(client)) {
        ...
        if (osMessageQueueGet(commandQueue, &command, NULL, 0) == osOK &&
            ApplyCommand(client, &command) != MQTTCLIENT_SUCCESS)
          break;
        Reading reading;
        if (osMessageQueueGet(queue, &reading, NULL, 0) == osOK) {
          if ((osKernelGetTickCount() - reading.tick) / osKernelGetTickFreq() >
              10)
            continue;
          char payload[384];
          int length =
              snprintf(payload, sizeof(payload),
                       ...
                       "\"temperature\":%.2f,\"humidity\":%.2f,\"gas_"
                       "resistance\":%.3f}}]}",
                       ENV_SERVICE_ID, reading.temperature, reading.humidity,
                       reading.gas);
          ...
          rc = MQTTClient_publishMessage(
              client, "$oc/devices/" ENV_DEVICE_ID "/sys/properties/report",
              &message, &token);
          if (rc == MQTTCLIENT_SUCCESS)
            rc = MQTTClient_waitForCompletion(client, token, 10000);
          if (rc != MQTTCLIENT_SUCCESS)
...
void EnvironmentCloudInit(void) {
  ...
  if (UtilsGetValue("env.thresholds", saved, sizeof(saved)) >= 0 &&
      sscanf(saved, "%d,%f,%f,%d,%f,%f,%d,%f,%f", &restored.temperatureEnabled,
             &restored.temperatureLow, &restored.temperatureHigh,
             &restored.humidityEnabled, &restored.humidityLow,
             &restored.humidityHigh, &restored.gasEnabled, &restored.gasLow,
             &restored.gasHigh) == 9 &&
  ...
  if (!queue || !commandQueue || !thresholdsMutex)
    return;
  osThreadAttr_t attr = {0};
  attr.name = "EnvironmentCloud";
  attr.stack_size = 10240;
  attr.priority = osPriorityNormal;
  if (!osThreadNew(CloudTask, NULL, &attr)) {
    osMessageQueueDelete(queue);
    queue = NULL;
  }
}
static void QueueReading(float temperature, float humidity, float gasResistance,
                         unsigned int now) {
  if (!queue)
    return;
...
void EnvironmentCloudReport(float temperature, float humidity,
  ...
  if ((now - lastQueuedTick) / osKernelGetTickFreq() <
      ENV_REPORT_INTERVAL_SECONDS)
    return;
  lastQueuedTick = now;
  QueueReading(temperature, humidity, gasResistance, now);
}
...
void EnvironmentCloudReportNow(float temperature, float humidity,
  ...
  lastQueuedTick = now;
  QueueReading(temperature, humidity, gasResistance, now);
  printf("[environment] alarm transition queued for immediate report\n");
}
